  ...
  gitlab: {
    ...
    actions: [
      ...
      },
      {
        ...
        connector: 'gitlab',
        displayName: 'Check MR Commit Author',
        description: 'Check if a specific user is the author or committer of any commit in a GitLab Merge Request',
        params: [
          { name: 'project_id', type: 'string', required: true, description: 'GitLab project ID or path' },
          { name: 'merge_request_iid', type: 'string', required: true, description: 'MR internal IID' },
          { name: 'user', type: 'string', required: false, description: 'Target user (email, name, or username)' },
          { name: 'email', type: 'string', required: false, description: 'Target author or committer email' },
          { name: 'author_name', type: 'string', required: false, description: 'Target author display name' },
          { name: 'message_contains', type: 'string', required: false, description: 'Optional commit message substring' },
          { name: 'message_regex', type: 'string', required: false, description: 'Optional commit message regex' },
        ],
        outputDescription: 'Result object with is_author boolean, match count, and commit list',
        exampleOutput: {
          is_author: true,
          found: true,
          match_count: 1,
          total_commits: 3,
          matched_commits: [{ id: 'abc12345', title: 'feat: add auth', author_name: 'Alice', author_email: 'alice@company.com' }],
          all_authors: ['Alice', 'Bob'],
          all_author_emails: ['alice@company.com', 'bob@company.com'],
        },
      },
      {
        action: 'gitlab.get_mr_commits',
        connector: 'gitlab',
        displayName: 'Get MR Commits',
        description: 'Fetch all commits and parsed author lists for a GitLab Merge Request',
        params: [
          { name: 'project_id', type: 'string', required: true, description: 'GitLab project ID or path' },
          { name: 'merge_request_iid', type: 'string', required: true, description: 'MR internal IID' },
          { name: 'per_page', type: 'number', required: false, description: 'Max commits to retrieve (default: 100)' },
        ],
        outputDescription: 'Object with total count, commit list, and unique author arrays',
        exampleOutput: {
          total: 2,
          commits: [{ id: 'abc12345', title: 'feat: auth', author_name: 'Alice' }],
          all_authors: ['Alice', 'Bob'],
          all_author_emails: ['alice@company.com', 'bob@company.com'],
        },
      },
  ...
  jira: {
    ...
    actions: [
      ...
      },
      {
        ...
        connector: 'jira',
        displayName: 'Check User Comment',
        description: 'Check if a specific user (by email, account ID, or name) has commented on an issue',
        params: [
          { name: 'issue_key', type: 'string', required: true, description: 'Jira issue key (e.g. PROJ-101)' },
          { name: 'user', type: 'string', required: false, description: 'User identifier (email, accountId, displayName, or username)' },
          { name: 'account_id', type: 'string', required: false, description: 'Explicit Atlassian accountId' },
          { name: 'email', type: 'string', required: false, description: 'Explicit author email address' },
          { name: 'display_name', type: 'string', required: false, description: 'Explicit author display name' },
          { name: 'body_contains', type: 'string', required: false, description: 'Optional comment body substring to match' },
          { name: 'body_regex', type: 'string', required: false, description: 'Optional regex pattern for comment body' },
        ],
        outputDescription: 'Result object with commented boolean, match count, and matched comment objects',
        exampleOutput: {
          commented: true,
          found: true,
          match_count: 1,
          total_comments: 2,
          matched_comments: [{ id: '1001', author_name: 'Alice Smith', author_email: 'alice@example.com', body_text: 'LGTM' }],
      ...
      },
      {
        ...
        description: 'Fetch all comments and author metadata for a Jira issue',
        params: [
          { name: 'issue_key', type: 'string', required: true, description: 'Jira issue key (e.g. PROJ-101)' },
          { name: 'max_results', type: 'number', required: false, description: 'Max comments to retrieve (default: 100)' },
        ],
        outputDescription: 'Object with total count, comment list, and unique author arrays',
        exampleOutput: {
          total: 2,
          comments: [{ id: '1001', author_name: 'Alice', body_text: 'LGTM' }],
          all_authors: ['Alice', 'Bob'],
